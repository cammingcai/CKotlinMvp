...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private Paint mCoustomPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint mImagePaint = new Paint();
//    private Paint mCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Path mTrianglePath = new Path();
    private Path mImagePath = new Path();
//    private Path mCirclePath = new Path();
    private static final int STYLE_NORMAL = 0;
    private static final int STYLE_TRIANGLE = 1;
    private static final int STYLE_BLOCK = 2;
    private static final int STYLE_IMAGE = 3;
    private static final int STYLE_COUSTOM = 4;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextMaxSize = ta.getDimension(R.styleable.SlidingTabLayout_tl_textMaxsize, sp2px(18));
//        mTextMaxSize = sp2px(18);
        ...
        mTextAnimator = ta.getBoolean(R.styleable.SlidingTabLayout_tl_textAnimator, false);
        ...
        mIconVisible = ta.getBoolean(R.styleable.SlidingTabLayout_tl_left_iconVisible, false);
  /*      mIconWidth = ta.getDimension(R.styleable.SlidingTabLayout_tl_iconWidth, dp2px(16));
        mIconHeight = ta.getDimension(R.styleable.SlidingTabLayout_tl_iconHeight, dp2px(16));*/
        ta.recycle();
    }
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            LinearLayout ll_tab =  v.findViewById(R.id.ll_tab);
            if(mIconVisible){
                ImageView tv_tab_icon =  v.findViewById(R.id.iv_tab_icon);
                if(mTabEntitys.get(i).getTabSelectedIcon()>0){
                    tv_tab_icon.setVisibility(View.VISIBLE);
                    tv_tab_icon.setImageResource(i==mCurrentTab?mTabEntitys.get(i).getTabSelectedIcon():mTabEntitys.get(i).getTabUnselectedIcon());
                }else{
                    tv_tab_icon.setVisibility(View.GONE);
                }
            }
            ...
            if (tv_tab_title != null) {
                ...
                if(mTextAnimator){
                    tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX,i == mCurrentTab ? mTextMaxSize:mTextsize);
                    tv_tab_title.getPaint().setFakeBoldText(i == mCurrentTab);
                }else{
                    tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize);
                }
                if(mIconVisible){
                    ll_tab .setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
    ...
    private void scrollToCurrentTab() {
        ...
        if(mTextAnimator){
            if (mCurrentTab > 0 || mCurrentPositionOffset > 0) {

                float sizeLeft = mTextMaxSize-(mTextMaxSize-mTextsize)*mCurrentPositionOffset;
                TextView titleLeft =  mTabsContainer.getChildAt(mCurrentTab).findViewById(R.id.tv_tab_title);

                titleLeft.setTextSize(TypedValue.COMPLEX_UNIT_PX,sizeLeft);

                if(mCurrentTab<mTabsContainer.getChildCount()-1){
                    float sizeRight = mTextsize+(mTextMaxSize-mTextsize)*mCurrentPositionOffset;
                    TextView titleRight =  mTabsContainer.getChildAt(mCurrentTab+1).findViewById(R.id.tv_tab_title);
                    titleRight.setTextSize(TypedValue.COMPLEX_UNIT_PX,sizeRight);
                }
            }
        }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if(mIconVisible){
                ImageView tv_tab_icon =  tabView.findViewById(R.id.iv_tab_icon);
                if(mTabEntitys.get(i).getTabSelectedIcon()>0){
                    tv_tab_icon.setVisibility(View.VISIBLE);
                    tv_tab_icon.setImageResource(isSelect?mTabEntitys.get(i).getTabSelectedIcon():mTabEntitys.get(i).getTabUnselectedIcon());
                }else{
                    tv_tab_icon.setVisibility(View.GONE);
                }
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        } else if (mIndicatorStyle == STYLE_COUSTOM){
            if (mIndicatorHeight > 0) {
                mCoustomPaint.setColor(mIndicatorColor);
                mCoustomPaint.setStyle(Paint.Style.STROKE);
                mCoustomPaint.setAntiAlias(true);
                mCoustomPaint.setStrokeWidth(10);
                RectF oval = new RectF(paddingLeft + mIndicatorRect.left,0,paddingLeft + mIndicatorRect.right,height);
                canvas.drawArc(oval,0,360,false,mCoustomPaint);
            }


        }else if (mIndicatorStyle == STYLE_IMAGE){
            if (mIndicatorHeight > 0) {
                mImagePath.reset();
//                mCirclePath.reset();
                mImagePaint.setStrokeWidth(10);
                ...
                mImagePaint.setColor(mIndicatorColor);
//                mCirclePaint.setColor(mIndicatorColor);


                float coustomWH = height/2- mIndicatorMarginBottom;
                float coustomLeft = paddingLeft + mIndicatorRect.left + (mIndicatorRect.width()-coustomWH)/2;

                RectF oval = new RectF(coustomLeft-coustomWH/2,height-coustomWH-coustomWH- mIndicatorMarginBottom,
                        coustomLeft+coustomWH+coustomWH/2,height- mIndicatorMarginBottom);

                float x = coustomLeft+coustomWH/2;
                float y = height-coustomWH- mIndicatorMarginBottom;
                float r = coustomWH;

                int startAngle = 70;
                int sweepAngle = 40;
                mImagePath.arcTo(oval,startAngle,sweepAngle,false);


//                float x1 = (float) (r*Math.cos(Math.toRadians(startAngle))+x);
//                float y1 = (float) (r*Math.sin(Math.toRadians(startAngle))+y);
//                mCirclePath.addCircle(x1,y1,5,Path.Direction.CW);
//                float x2 = (float) (r*Math.cos(Math.toRadians(startAngle+sweepAngle))+x);
//                float y2 = (float) (r*Math.sin(Math.toRadians(startAngle+sweepAngle))+y);
//                mCirclePath.addCircle(x2,y2,5,Path.Direction.CW);
                //封闭形成闭环
//                mImagePath.close();
